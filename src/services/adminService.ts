...
/**
 * Fetches all subjects (active and inactive) with joined university and academic year.
 */
...
/**
 * Updates an existing subject's details (price, names, active state).
 */
export async function updateSubject(
  ...
  if (error) {
    ...
    if (error.message.includes("uq_subject_uni_year_name") || error.code === "23505") {
      throw new Error("توجد مادة مسجلة مسبقاً بنفس الاسم لهذه الجامعة والفرقة المختارة.");
    }
...
export async function createSubject(input: SubjectMutationInput): Promise<string> {
  ...
  if (error) {
    ...
    if (error.message.includes("uq_subject_uni_year_name")) {
      throw new Error("توجد مادة مسجلة مسبقاً بنفس الاسم لهذه الجامعة والفرقة.");
    }
...
export async function fetchAdminStats(): Promise<AdminStats> {
  const [subjectsRes, unisRes, yearsRes] = await Promise.all([
