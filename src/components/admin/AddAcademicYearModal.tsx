...
import { createAcademicYear } from "@/services/adminService";
import { AcademicYearMutationInput } from "@/types";
...
export default function AddAcademicYearModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "فشل إضافة الفرقة الدراسية.");
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-slate-900">
                إضافة فرقة دراسية جديدة
              </h2>
              <p className="text-xs text-slate-400">
                إضافة فرقة أو سنة أكاديمية للنظام
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              يحدد ترتيب ظهور الفرقة الدراسية في القوائم واستمارة التسجيل.
            </p>
          </div>
          ...
          <div className="flex items-center justify-between p-3.5 rounded-2xl bg-slate-50 border border-slate-100">
            <div>
              <p className="text-xs font-bold text-slate-800">تفعيل الفرقة فوراً</p>
              <p className="text-[11px] text-slate-400">
                إتاحة الفرقة للطلاب في استمارة التسجيل وقوائم المواد
              </p>
            </div>
